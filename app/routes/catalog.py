...
def add_copy(book_id):
    ...
    if request.method == 'POST':
        ...
        existing = BookCopy.query.filter_by(accession_number=accession).first()
        if existing:
            flash('Accession number already exists', 'error')
        ...
        flash(f'Copy {copy.accession_number} added successfully', 'success')
        return redirect(url_for('catalog.view_book', book_id=book.id))
...
def edit_copy(copy_id):
    ...
    if request.method == 'POST':
        copy.barcode = request.form.get('barcode', '').strip() or None
...
@catalog_bp.route('/api/copy/<accession>')
@login_required
def api_get_copy(accession):
